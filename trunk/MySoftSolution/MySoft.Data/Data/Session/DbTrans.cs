...
namespace MySoft.Data
{
    ...
    public class DbTrans : IDbTrans, IDisposable
    {
        ...
        /// <param name="values"></param>
        /// <returns></returns>
        public int Insert<T>(Table table, Field[] fields, object[] values, out object retVal)
            where T : Entity
        {
            return dbBatch.Insert<T>(table, fields, values, out retVal);
        }
        ...
        /// <param name="values"></param>
        /// <returns></returns>
        public int Insert<T>(Field[] fields, object[] values, out object retVal)
            where T : Entity
        {
            return dbBatch.Insert<T>(fields, values, out retVal);
        }
        ...
        public int Excute(InsertCreator creator)
        {
            ...
            object retVal;
            return dbProvider.Insert<TempEntity>(creator.Table, creator.FieldValues, this, creator.IdentityField, creator.SequenceName, false, out retVal);
        }
        ...
        public int Excute<TResult>(InsertCreator creator, out TResult identityValue)
        {
            ...
            {
                ret = dbProvider.Insert<TempEntity>(creator.Table, creator.FieldValues, this, creator.IdentityField, creator.SequenceName, true, out retVal);
        ...
        public int Excute(DeleteCreator creator)
        {
            ...
            return Delete<TempEntity>(creator.Table, creator.Where);
        }
        ...
        public int Excute(UpdateCreator creator)
        {
            ...
            return Update<TempEntity>(creator.Table, creator.Fields, creator.Values, creator.Where);
        }
        ...
        public QuerySection From(QueryCreator creator)
        {
            ...
            FromSection<TempEntity> f = this.From<TempEntity>(creator.Table);
            if (creator.IsRelation)
            {
                foreach (TableJoin join in creator.Relations.Values)
                {
                    if (join.Type == JoinType.LeftJoin)
                        f.LeftJoin<TempEntity>(join.Table, join.Where);
                    else if (join.Type == JoinType.RightJoin)
                        f.RightJoin<TempEntity>(join.Table, join.Where);
                    else
                        f.InnerJoin<TempEntity>(join.Table, join.Where);
                }
            }

            QuerySection<TempEntity> query = f.Select(creator.Fields).Where(creator.Where)
