...
namespace KiShion.Tools.EntityDesign
{
    public class CodeGenHelper
    {
        ...
        public string GenEntitiesEx(Assembly ass, int outLang)
        {
            ...
            foreach (Type type in ass.GetTypes())
            {
                if (typeof(IEntity).IsAssignableFrom(type) && typeof(IEntity) != type && advOpt.IsEntityEnabled(type.Name))
                {
                    ...
                    ns.Imports.Add(new CodeNamespaceImport("KiShion.Data"));
                    //ns.Imports.Add(new CodeNamespaceImport("KiShion.Data.Design"));
        ...
        private KiShion.Data.Field GetField(Type entityType, PropertyInfo property)
        {
            //获取字段名
            string fieldName = null;
