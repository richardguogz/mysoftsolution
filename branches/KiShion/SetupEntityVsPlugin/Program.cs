...
namespace SetupNBearVsPlugin
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            string addInFile = vsNetAddInDir + "\\KiShion.Tools.EntityDesignVsPlugin.AddIn";
            ...
            if (Directory.Exists(vsNetAddInDir))
            {
                if (args != null && args.Length > 0)
                {
                    if (args[0].ToLower() == "-u")
                    {
                        ...
                        MessageBox.Show("卸载KiShion实体生成插件成功！", "卸载插件", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
                    try
                    {
                        ...
                        if (existingAddIns != null)
                        {
                            ...
                            {
                                if (addin.Contains("KiShion.Tools.EntityDesignVsPlugin - For Testing.AddIn"))
                                {
                        ...
                        string content = File.ReadAllText("KiShion.Tools.EntityDesignVsPlugin.AddIn");
                        content = string.Format(content, visionNo, AppDomain.CurrentDomain.BaseDirectory.TrimEnd('\\'));
                        File.WriteAllText(addInFile, content);

                        MessageBox.Show("安装KiShion实体生成插件成功！", "安装插件", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception ex)
                    {
