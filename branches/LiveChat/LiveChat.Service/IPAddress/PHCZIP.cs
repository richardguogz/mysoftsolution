...
namespace LiveChat.Service
{
    ...
    public class PHCZIP
    {
        private readonly static PHCZIP Instance = new PHCZIP();

        public static String Get(String ip)
        {
            string area = Instance.GetAddressWithIP(ip);

            if (string.IsNullOrEmpty(area)) return area;

            //只返回省市
            return area.Replace("CZ88.NET", string.Empty).Trim().Split(' ')[0];
        }
        ...
        public PHCZIP()
        {
            ...
            SetDbFilePath(AppDomain.CurrentDomain.BaseDirectory + "\\Data\\QQWry.Dat");
        }
